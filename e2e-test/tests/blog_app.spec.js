  ...
  describe("Login", () => {
    test("succeeds with correct credentials", async ({ page }) => {
      ...
      await expect(notificationDiv).toContainText(
        "Flavie Qin successfully signed in"
      );
  ...
  describe("When logged in", () => {
    ...
    describe("Many blogs already exist", () => {
      ...
      test("blogs are listed in order of likes", async ({ page }) => {
        ...
        await thirdBlog.getByText("1 like").waitFor(); //wait for the blog to be updated
        await thirdBlog.getByRole("button", { name: "Like" }).click();
      ...
      describe("when a different user is logged in", () => {
        ...
        test("cannot see the delete button on blog that did not create", async ({
          page,
        }) => {
          const blog = page.getByText("First blog, by Me");
          await blog.getByRole("button", { name: "View" }).click();
